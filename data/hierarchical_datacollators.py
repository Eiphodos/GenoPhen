...
@dataclass
class HierDataCollatorWithPadding:
    """
...
@dataclass
class HierSumDataCollatorWithPadding:
    """
    Data collator that will dynamically pad the inputs received.
    ...
    def __call__(self, features: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        gene_ids = [v.pop('gene_ids') for v in features]
        max_len_gi = max([len(gii) for gi in gene_ids for gii in gi])
        ...
        bgi = torch.full((batch_size, max_len_ii, max_len_gi), self.tokenizer.pad_token_id, dtype=torch.int32)
        for gi in range(len(gene_ids)):
            for gii in range(len(gene_ids[gi])):
                bgi[gi, gii, 0:len(gene_ids[gi][gii])] = torch.IntTensor(gene_ids[gi][gii])
        batch['gene_ids'] = bgi
...
@dataclass
class HierDataCollatorForLanguageModeling(DataCollatorMixin):
    """
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        if isinstance(examples[0], Mapping):
            batch = self.tokenizer.pad(input_ids, return_tensors="pt", pad_to_multiple_of=self.pad_to_multiple_of)
        else:
            batch = {
...
@dataclass
class HierSumDataCollatorForLanguageModeling(DataCollatorMixin):
    """
    Data collator used for language modeling. Inputs are dynamically padded to the maximum length of a batch if they
    are not all of the same length.
    ...
    def torch_call(self, examples: List[Union[List[int], Any, Dict[str, Any]]]) -> Dict[str, Any]:
        ...
        # Handle dict or lists with proper padding and conversion to tensor.
        input_ids = [{'input_ids': v['input_ids']} for v in examples]
        gene_ids = [v['gene_ids'] for v in examples]
        ...
        bgi = torch.full((len(examples), max_len_ii, max_len_gi), self.tokenizer.pad_token_id, dtype=torch.int32)
        for gi in range(len(gene_ids)):
            for gii in range(len(gene_ids[gi])):
                bgi[gi, gii, 0:len(gene_ids[gi][gii])] = torch.IntTensor(gene_ids[gi][gii])
        batch['gene_ids'] = bgi
        ...
        special_tokens_mask = batch.pop("special_tokens_mask", None)
        if self.mlm:
            batch["input_ids"], batch["labels"], batch['gene_ids'] = self.torch_mask_tokens(
                batch["input_ids"], batch["gene_ids"], special_tokens_mask=special_tokens_mask
            )
        else:
            labels = batch["input_ids"].clone()
            if self.tokenizer.pad_token_id is not None:
    ...
    def torch_mask_tokens(self, inputs: Any, gene_ids: Any, special_tokens_mask: Optional[Any] = None) -> Tuple[Any, Any]:
        """
        Prepare masked tokens inputs/labels for masked language modeling: 80% MASK, 10% random, 10% original.
        """
        import torch
        ...
        inputs[indices_replaced] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
        gene_ids[indices_replaced,:] = self.tokenizer.convert_tokens_to_ids(self.tokenizer.mask_token)
...
@dataclass
class DataCollatorForWholeWordMask(HierDataCollatorForLanguageModeling):
    """
...
@dataclass
class DataCollatorForSOP(HierDataCollatorForLanguageModeling):
    """
